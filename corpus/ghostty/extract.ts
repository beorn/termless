...
function tryConvertTerminalTest(raw: RawTestBlock): ConvertResult {
  ...
  const initMatch = /\b(?:var|const)\s+t\s*=\s*try\s+init\(\s*([\w.]+)\s*,\s*\.\{([^}]*)\}\s*\)/.exec(
    masked,
  );
  if (!initMatch) {
  ...
  if (pieces.length !== rawPrintCallCount) {
    return {
      reason:
        "print/printString called with a non-literal argument (variable, numeric codepoint, or other expression)",
    };
  }
...
function main(): void {
  ...
  for (const fileName of SOURCE_FILES) {
    ...
    console.log(
      `${fileName}: ${blocks.length} test blocks, ${convertedCount} auto-converted`,
    );
  }
