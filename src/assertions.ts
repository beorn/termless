...
export function isCellView(value: unknown): value is CellView {
  return (
    value !== null && typeof value === "object" && "bold" in value && "fg" in value && "row" in value && "col" in value
  )
}
...
export function assertIsStrikethrough(cell: CellView): AssertionResult {
  ...
  return {
    ...
    message: cell.strikethrough ? `Expected ${loc} not to be strikethrough` : `Expected ${loc} to be strikethrough`,
  }
}
...
export function assertCursorAt(term: TerminalReadable, x: number, y: number): AssertionResult {
  ...
  return {
    ...
    message: pass
      ...
      : `Expected cursor at (${x},${y}), got (${cursor.col},${cursor.row})`,
    expected: { x, y },
    actual: { x: cursor.col, y: cursor.row },
  }
}
...
export function assertHaveCursor(term: TerminalReadable, props: CursorProps): AssertionResult {
  ...
  if (props.x !== undefined) check("x", props.x, cursor.col, cursor.col === props.x)
  if (props.y !== undefined) check("y", props.y, cursor.row, cursor.row === props.y)
...
export function assertInMode(term: TerminalReadable, mode: TerminalMode): AssertionResult {
  ...
  return {
    pass,
    message: pass ? `Expected terminal not to be in mode "${mode}"` : `Expected terminal to be in mode "${mode}"`,
  }
}
...
export function assertAtBottomOfScrollback(term: TerminalReadable): AssertionResult {
  ...
  return {
    ...
    message: pass
      ...
      : `Expected terminal to be at bottom of scrollback (viewportTop=${bottomOffset}), got ${scrollback.viewportTop}`,
  }
}
