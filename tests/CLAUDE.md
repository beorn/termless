...
- **SVG rendering**: `screenshotSvg()` output validity, cell styling (bold, italic, dim, fg/bg color, inverse, underline, strikethrough), cursor styles (block, beam, underline), custom themes/fonts, XML escaping, dimension calculation, bg-rect merging
...
```bash
bun vitest run tests/                    # All root tests
bun vitest run tests/terminal.test.ts    # Key mapping + Terminal API
bun vitest run tests/svg.test.ts         # SVG screenshot renderer
bun vitest run tests/integration.test.ts # Full-stack integration
bun vitest run tests/cross-backend.test.ts # Cross-backend conformance
```
