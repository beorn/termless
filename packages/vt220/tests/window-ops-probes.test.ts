/**
 * Window-op probe responses — vt220 backend.
 *
 * vt220 is a pure-TypeScript cell-grid emulator with no native window
...
describe("window-op probe responses — vt220 backend", () => {
  test("answers CSI 14t with text-area pixel size (CSI 4;h;w t)", async () => {
    const responses = await probeBackend(createVt220Backend(), "\x1b[14t")
  ...
  test("answers CSI 18t with text-area cell count (CSI 8;h;w t)", async () => {
    const responses = await probeBackend(createVt220Backend(), "\x1b[18t")
