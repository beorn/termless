...
export async function startMcpServer(): Promise<void> {
  ...
  register(
    ...
    {
      description:
        "Open a live terminal session — a TestTerminal object backed by a PTY and a headless terminal emulator backend. Default backend is xtermjs (fast, portable, lower visual fidelity). Use 'ghostty' for visual-faithful screenshots (full glyph coverage matching the real Ghostty terminal) — required for visual-bug-close Layer 2 evidence. PALETTE: the PTY DEFAULTS a child to truecolor (FORCE_COLOR=3) and only fills in what the environment does not already say — an ambient FORCE_COLOR or NO_COLOR is respected, and your explicit env beats both. Still say which palette a colour finding was taken at: at 16 colours a Nord theme collapses (blue and error-red both go grey), so a black ground with greys of 128/192 is a palette signature rather than a finding about the app. Pass env: { FORCE_COLOR: '1' } to capture deliberately narrow. Pass `trace: { dir }` to also *record* the session: every buffer mutation is captured as a debounced PNG + JSONL frame. Read the recorded frames with the `trace` tool; finalize the recording with `stop`.",
      inputSchema: {
        ...
        backend: z
          ...
          .optional()
          .describe(
            "TestTerminal emulator backend. 'vterm' (default) — pure-TS, standards-compliant, the production engine; reports cursor shape, cursor visibility and narrowing reflow faithfully. 'xtermjs' — the differential reference; fast and portable, but flattens those three. 'ghostty' — ghostty-web WASM, full glyph coverage, matches real Ghostty rendering (use for visual-bug screenshots); it can DISPLAY truecolor and the child now defaults to it, but an ambient FORCE_COLOR or NO_COLOR still wins — say which palette a colour finding was taken at. 'vt100' — minimal VT100 subset. 'peekaboo' — OS automation against a real terminal app (macOS only, slowest, pixel-perfect). An unknown name is refused, never substituted.",
          ),
        trace: z
          .object({
