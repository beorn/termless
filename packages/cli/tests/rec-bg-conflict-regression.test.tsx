 ...
 * background coherence: when ANSI bg in text content layers over an silvery
 * `backgroundColor`, it `throw`s a "Background conflict" by default — the
 * right safety net for an silvery app's own pipeline bugs.
 *
 ...
 * conflict-rich by nature. The original `<Terminal>` rewire
 * (termless `2cc705d`) crashed `termless rec` for any real TUI:
 *
 *   [silvery] Background conflict at (1,11): chalk bg=#c5cbd7 on silvery
 *   bufferBg=rgb(0,0,0). …
 *
 ...
 *
 * Pairs with `vendor/silvery/tests/features/terminal-component.test.tsx`
 * (the synthetic-grid component tests) and bead
 * `@km/silvery/15506-terminal-bg-conflict-rec-regression`.
 */
...
describe("rec-live-overlay — <Terminal> vs. real km-view ANSI", () => {
  ...
  test("<Terminal> renders real km-view ANSI inside a bg Box WITHOUT throwing", async () => {
    ...
    // The `<Box backgroundColor>` is the conflict trigger: it seeds an
  ...
  test('<Terminal>\'s own rows opt out via bgConflict="ignore"', async () => {
    ...
    // The km-view chrome rendered — sanity-check a stable substring.
