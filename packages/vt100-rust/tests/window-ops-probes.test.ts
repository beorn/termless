/**
 * Window-op probe responses — vt100-rust backend.
 *
 * doy/vt100 (Rust via napi-rs) is a pure VT parser with no native window
 * pixel concept, so we synthesize CSI 14t (text-area pixel size) and
 * CSI 18t (text-area cell count) responses from configured grid × cell
...
import { createVt100RustBackend, loadVt100RustNative } from "../src/backend.ts"
...
describeNative(`window-op probe responses — vt100-rust backend${skipReason ? ` (skipped: ${skipReason})` : ""}`, () => {
  test("answers CSI 14t with text-area pixel size (CSI 4;h;w t)", async () => {
    const responses = await probeBackend(createVt100RustBackend(), "\x1b[14t")
  ...
  test("answers CSI 18t with text-area cell count (CSI 8;h;w t)", async () => {
    const responses = await probeBackend(createVt100RustBackend(), "\x1b[18t")
