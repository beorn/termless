...
```bash
bun vitest run packages/viterm/tests/                   # All @termless/test tests
bun vitest run packages/viterm/tests/matchers.test.ts   # Matcher pass/fail
bun vitest run packages/viterm/tests/fixture.test.ts    # Fixture auto-cleanup
bun vitest run packages/viterm/tests/serializer.test.ts # Snapshot format
```
