...
function convertGhosttyCell(
	...
	const fg: RGB | null =
		isDefaultColor(cell.fg_r, cell.fg_g, cell.fg_b, defaults, true)
			? null
			: { r: cell.fg_r, g: cell.fg_g, b: cell.fg_b }

	const bg: RGB | null =
		isDefaultColor(cell.bg_r, cell.bg_g, cell.bg_b, defaults, false)
			? null
...
export function createGhosttyBackend(
	...
	function init(options: TerminalOptions): void {
		...
		term.update()
		const colors = term.getColors() as { foreground: { r: number; g: number; b: number }; background: { r: number; g: number; b: number } }
		defaultColors = {
