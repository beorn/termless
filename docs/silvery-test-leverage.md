...
silvery has ~170 test files in `vendor/silvery/tests/`. The primary testing approach is virtual buffer testing via `createRenderer` from `silvery/testing`, which renders React components to an in-memory `TerminalBuffer` and asserts against plain text (`app.text`) or buffer cells (`app.term.cell(x, y)`).
