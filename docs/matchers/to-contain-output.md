...
```typescript
expect(term.output).toContainOutput(text: string, options?: { timeout?: number })
```
...
expect(term.output).toContainOutput("\x1b_Ga=p")
...
- `term.output` is raw output before terminal parsing.
